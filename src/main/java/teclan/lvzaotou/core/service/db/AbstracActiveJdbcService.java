package teclan.lvzaotou.core.service.db;
...
import teclan.lvzaotou.core.db.Page;

public abstract class AbstracActiveJdbcService<T extends ActiveRecord> implements ActiveJdbcService<T> {
	...
	public Page<T> fetch(int page, int limit, String query, Object... params) {
		...
		Paginator<T> paginator = new Paginator<>(getModelClass(), limit, query, params).orderBy("created_at DESC");
		return new Page<>(paginator.getCount(), page, limit, paginator.getPage(page));
	}
	...
	private void deleteAll(List<Long> toBeRemoved) {
		if (toBeRemoved.isEmpty()) {
			ModelDelegate.findAll(getModelClass()).forEach(r -> toBeRemoved.add(r.getLongId()));
		}
	...
	@Override
	public T sync(long id, Map<String, Object> attributes) {
		return update(id, attributes);
	}
